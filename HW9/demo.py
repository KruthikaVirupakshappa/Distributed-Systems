...
class DatabaseWithCache:
    ...
    def increment_post_views(self, post_id: int) -> int:
        """Increment view count in Redis; sync to database every 10 views"""
...
def run_demo():
    ...
    print("=" * 60)
    db.clear_all_cache()
    db.redis_client.delete("views:post:1")  # Ensure counter starts at 0

    for i in range(1, 26):
        views = db.increment_post_views(1)
        print(f"View #{i}: Total views = {views}")
        if views % 10 == 0:
